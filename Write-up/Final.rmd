...
Another feature of the marker map is that if a circle is clicked, a pop-up display will show up. An example is given below.

![Figure 6](Shiny_Screenshots/marker3.PNG)

The pop-up displays relevant information regarding the accident including whether a drunk driver was involved, the number of vehicles, the number of people, and the number of fatalities. The most important feature of the pop-up is that it displays the unique ID given to each accident. If there is a particularly interesting accident displayed on the marker map and the user would like to look into it, the ID provided the pop-up is essential. The user could take the ID, and match the accident from the map to the corresponding row in the dataset.

### Choropleth Map 1

The second of the three components was a choropleth map. The primary use of this component is to see how fatal traffic accidents involving drunk drivers varies from county to county and to explore possible relationships between drunk driving and other predictors. The user could select they wanted a state or a county level choropleth map. The user could also select whether they wanted the choropleth to be based on raw count of accidents or number of accidents per 100,000. Other features include selecting whether the data contained all accidents or only those involving drunk driver; whether to include accidents occurring during the weekend, the weekday, or both; and whether to include accidents occurring during the night, the day, or both. 

Displayed below is a county level choropleth map of raw count of fatal accidents involving drunk drivers. The darker the green the more fatal accidents involving a drunk driver in that county.

![Figure 7](Shiny_Screenshots/choropleth1.PNG)

Displayed below is a county level choropleth map of the number of fatal accidents involving drunk drivers per 100,000 inhabitants. This was to normalize for population as the larger counties are expected to have larger raw counts.

![Figure 8](Shiny_Screenshots/choropleth2.PNG)

The two visuals above confirm that fatal accidents involving drunk drivers is not uniformly distributed both in terms of raw count and rate. From the differences in the first visual to the second, it seems that those counties with large populations (and therefore more raw accidents) seem to have more stable rates of fatal accidents involving drunk drivers. These displays also serve to point out the counties and regions with abnormally high rates of fatal traffic accidents involving drunk drivers. In particular, the Mountain West region seems to have the highest share of counties that are very green (and so have the largest rates of accidents involving drunk drivers).

This next display is a state level county level choropleth map of the number of fatal accidents involving drunk drivers per 100,000 inhabitants.

![Figure 9](Shiny_Screenshots/choropleth3.PNG)
...
A nice feature of the second component of the shiny app is that it allows the users to zoom in on states they wish to explore further. As an example, displayed below are choropleth plots zoomed in on the states of New Jersey and Wyoming. Morris County and Fremont County (the counties discussed in the motivation section) are marked by a blue X. 

![Figure 10](Shiny_Screenshots/zoom1.PNG)
![Figure 11](Shiny_Screenshots/zoom2.PNG)

The visual confirms that indeed the rates of fatal traffic accidents involving drunk drivers is much higher in Fremont County than in Morris County.
...
```{r}
modelAssess <- mergeData %>% dplyr::select(Age, PrevSuspensions, PrevDWIConvictions, 
...
```{r}
mergeData$DriverDrinking <- as.numeric(mergeData$DriverDrinking)
FinalData2$DriverDrinking <- as.numeric(FinalData2$DriverDrinking)
mergeData$DriverDrinking[mergeData$DriverDrinking == 1] <- 0
mergeData$DriverDrinking[mergeData$DriverDrinking == 2] <- 1
emplogit(mergeData$TotalPopulation, mergeData$DriverDrinking)
title(sub = "small")
emplogit(mergeData$IncomeToPovRatio, mergeData$DriverDrinking)
